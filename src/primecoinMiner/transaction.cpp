...
bitclientTransaction_t* bitclient_createCoinbaseTransactionFromSeed(uint32 seed, uint32 seed_userId, uint32 blockHeight, 
                                                                    uint8* pubKeyRipeHash, uint64 inputValue, double donatePercentage)
{
   ...
   bitclient_addScriptData(vInput, (uint8*)minerVersionString, strlen(minerVersionString), true);
   bitclient_addScriptData(vInput, (uint8*)"/P2SH/", 6, false); // coinbase flags...
   // create output
   uint64 donateAmount = (uint64)(inputValue * (donatePercentage/100.0));
   if (donateAmount > 0)
   {
      bitclientVOut_t* vOutputDonation = bitclient_addTransactionOutput(tx);
      bitclient_addScriptOpcode(vOutputDonation, 0x76); // OP_DUP
      bitclient_addScriptOpcode(vOutputDonation, 0xA9); // OP_HASH160
      bitclient_addScriptData(vOutputDonation, (uint8*)&developerPubKeyRipeHash[0], 20);
      bitclient_addScriptOpcode(vOutputDonation, 0x88); // OP_EQUALVERIFY
      bitclient_addScriptOpcode(vOutputDonation, 0xAC); // OP_CHECKSIG
      vOutputDonation->coinValue = donateAmount;
   }
   else
   {
      donateAmount  = 0;
   ...
   vOutput->coinValue = (inputValue - donateAmount);
   return tx;
}
